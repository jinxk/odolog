...
- **Reminders:** switches for document reminders (insurance, PUC, RC, fitness) and service reminders (oil change, general service), both on by default. Below them, what is scheduled in the categories that are on: one row per document or service and vehicle, with the next fire date and time and how many more follow, soonest first. A test row fires one notification straight away so the user can check reminders arrive on the phone, and a line appears when notifications are switched off for OdoLog in Android settings. The notification permission prompt appears the first time it is actually needed: the first document expiry date saved on a vehicle, the first service logged for a vehicle, or a tap on the test row, never at app start.
