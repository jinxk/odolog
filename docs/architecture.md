...
OdoLog is a single Flutter app targeting Android first. The fuel economy math is testable in isolation and the storage layer can be swapped without touching screens.
...
| Local database | sqflite | Mature SQLite binding for Flutter. Real SQL keeps the window and aggregate queries in the database, and an in-memory database makes repository tests fast. |
| Routing | go_router | Declarative routes, deep-link ready, works with a bottom navigation shell. |
...
| Value equality | equatable | Structural equality for domain models and stat value objects, which keeps Riverpod rebuilds accurate and tests readable. |

No dependency injection framework beyond Riverpod, no ORM, no reactive database wrapper.
...
Domain knows about neither Flutter nor SQLite. Every stat formula is unit testable with plain Dart and no database.
...
A non-fuel cost against a vehicle: a tyre change, a repair, an insurance premium, and so on. The category is free text, not a fixed enum, with suggestion chips in the UI (Service, Tyre, Repair, Insurance, Other).

### Computed value objects

These are outputs of the calculators, never persisted. A screen receives one typed object instead of a set of loose doubles.
...
`DocumentAlert` and `ServiceDueStatus` carry the remaining time a glance element reads. `DocumentReminder` and `ServiceReminder` are what gets handed to the `ReminderScheduler` port.

## Repository interfaces

Repositories return `Either<Failure, T>`, aliased as `Result<T>`. Callers handle failure explicitly instead of catching exceptions across layers.
...
Document expiry reminders and service due reminders both end at a local notification. `ReminderScheduler` is the port that keeps `flutter_local_notifications` out of the domain:
...
Each method is a full reconcile of its own category, not an append: the caller passes the complete set of future reminders and the implementation cancels anything in that category no longer in it. The two categories are reconciled independently, so syncing one never affects the other. `LocalNotificationScheduler` in `data/reminders/` is the only implementation, Android only, and best effort by contract: a platform that cannot schedule does nothing rather than failing the caller. The equivalent port for the backup file format, `DataBundleCodec`, follows the same shape; see the `ExportData` and `ImportData` use cases below.
...
The catalog lives at `assets/fuel_catalog.json`, registered in `pubspec.yaml`. `CatalogRepositoryImpl` loads it through `rootBundle`, parses it once, and caches the result for the session. Its own `version` field lets a future in-app update, or a shipped update, detect a newer catalog. Entries store the product `id` and not a label, so updating the catalog never invalidates existing history. A missing or unparseable asset degrades to an empty brand list plus the always-present "Other" free-text option, so fuel logging never blocks on the catalog.
...
- **NotFoundFailure** for a vehicle or entry id that does not resolve, usually a stale route argument.
...
Three tiers, weighted toward the math.
...
None of this is built in v1. iOS is a later target, and the layering keeps Flutter-specific code out of the domain, so the platform lift is build config and platform-channel details, not a rewrite. An EV category would add a `kwh` unit and a `charge` fuel category, which the unit-follows-category design already allows for; the window math is unit-agnostic and would report km/kWh without change. Both stay out of scope until the core logging and stats are in place.
